	...
	<?php
		...
		<div class="acymailing_fulldiv" id="acymailing_fulldiv_<?php echo $formName; ?>" <?php echo $styleString; ?> >
			...
			<div class="acymailing_module_form" >
				...
				?>
				<div class="c-footer-delivery__title"><?php echo JText::_('TAKE_LATEST_NEWS');?></div>
				<div class="l-footer-delivery">
					<input id="user_email_<?php echo $formName; ?>" <?php if(!empty($identifiedUser->userid)) echo '';  if(!$displayOutside){ ?> onfocus="if(this.value == '<?php echo $emailCaption;?>') this.value = '';" onblur="if(this.value=='') this.value='<?php echo $emailCaption?>';"<?php } ?> class="c-footer-delivery__field field_effect email_patternt" type="text" name="user[email]"  alue="" placeholder="E-mail" title="<?php echo $emailCaption;?>"/>

						<button type="submit" class="c-footer-delivery__but"></button>
				</div>
					...
					if(empty($identifiedUser->userid) AND $config->get('captcha_enabled') AND acymailing_level(1)){ ?>
					 ...
					 if($params->get('showterms',false)){
						...
						<td <?php if($displayOutside AND !$displayInline) echo 'colspan="2"'; ?> class="acysubbuttons">
							...
							<input class="button subbutton btn btn-primary _center" type="submit" value="<?php echo JText::_('TPL_SILV_SUBCRIP_HEAD_SHORT');?>" name="Submit" onclick="try{ return submitacymailingform('optin','<?php echo $formName;?>'); }catch(err){alert('The form could not be submitted '+err);return false;}"/>
