...
class N2SSPluginItemInput extends N2SSPluginItemAbstract
{
    ...
    function getTemplate($slider) {
        ...
        ), NHtml::tag('table', array(
            'style'       => 'width:auto',
            'cellpadding' => 0,
            'cellspacing' => 0
        ), NHtml::tag('tr', array(), NHtml::tag('td', array('style' => '{fullwidthStyle}'), "<div class='n2-fake-input {inputfontclass} {inputstyleclass}' style='width:100%;white-space:nowrap;'>{placeholder}</div>") . NHtml::tag('td', array(), NHtml::tag('div', array(
    ...
    function _renderAdmin($data, $itemId, $slider, $slide) {
        ...
        if (!empty($buttonLabel)) {
            ...
            $td2 = NHtml::tag('td', array(), NHtml::tag('div', array(
                'style' => 'white-space:nowrap;display:inline-block;',
        ...
        return NHtml::tag('div', array(
            ...
            'cellpadding' => 0,
            'cellspacing' => 0
        ), NHtml::tag('tr', array(), NHtml::tag('td', array('style' => ($data->get('fullwidth', 0) ? 'width:100%' : '')), "<div class='n2-fake-input " . $inputFont . " " . $inputStyle . "' style='width:100%;white-space:nowrap;'>" . strip_tags($slide->fill($data->get('placeholder', ''))) . "</div>") . $td2)));
    }
    ...
    private function getHtml($data, $id, $slider, $slide) {
        ...
        foreach ($parameters AS $parameter) {
            ...
            if (count($parameter) == 2) {
                $parametersHTML .= NHtml::tag('input', array(
                    'type'  => 'hidden',
                    'name'  => $parameter[0],
                    'value' => $parameter[1]
                ));
        ...
        if (!empty($buttonLabel)) {
            ...
            $td2 = NHtml::tag('td', array(), NHtml::tag('input', array(
        ...
        return NHtml::tag('form', array(
            ...
            'cellpadding' => 0,
            'cellspacing' => 0
        ), NHtml::tag('tr', array(), NHtml::tag('td', array('style' => ($data->get('fullwidth', 0) ? 'width:100%' : '')), NHtml::tag('input', array(
                    'name'        => $data->get('name', ''),
                    'type'        => 'text',
