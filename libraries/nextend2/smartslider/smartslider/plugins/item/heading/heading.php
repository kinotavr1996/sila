...
class N2SSPluginItemHeading extends N2SSPluginItemAbstract
{
    ...
    function getTemplate($slider) {
        ...
        return "<div><h{priority} id='{uid}' class='{fontclass} {styleclass} {class}' style='display: {display}; {extrastyle};'><a href='#' onclick='return false;'>{heading}</a></h{priority}>" . NHtml::scriptTemplate($this->getJs($slider->elementId, "{uid}")) . "</div>";
    }
